...
{   
    ...
    public class ModBuildScript : BuildScriptBase
    {
        /// <inheritdoc />
        public override string Name
        {
            get { return "Mod Build Script"; }
        }
        ...
        public ModExportConfig ExportConfig;
        ...
        protected override TResult BuildDataImplementation<TResult>(AddressablesDataBuilderInput builderInput)
        {
            ...
            CreateModOnlyBuildSetup(builderInput.AddressableSettings);
            ...
            if (result == null)
            {
                ...
                CleanupModSetup(aaContext.Settings);
            }
            ...
            out AddressableAssetsBuildContext aaContext)
        {
            ...
            var runtimeData = new ResourceManagerRuntimeData
            {
                ...
                AddressablesVersion = UnityEditor.PackageManager.PackageInfo
                    .FindForAssembly(typeof(Addressables).Assembly)?.version,
#endif
            ...
            m_Linker.AddAssemblies(new[]
                { typeof(Addressables).Assembly, typeof(UnityEngine.ResourceManagement.ResourceManager).Assembly });
            ...
            AddressableAssetsBuildContext aaContext) where TResult : IDataBuilderResult
        {
            ...
            if (m_AllBundleInputDefs.Count > 0)
            {
                ...
                {
                    var exitCode = ContentPipeline.BuildAssetBundles(buildParams,
                        new BundleBuildContent(m_AllBundleInputDefs), out results, buildTasks, aaContext, Log);
                ...
                using (var progressTracker = new UnityEditor.Build.Pipeline.Utilities.ProgressTracker())
                {
                    ...
                    Dictionary<string, ContentCatalogDataEntry> primaryKeyToCatalogEntry =
                        new Dictionary<string, ContentCatalogDataEntry>();
                    foreach (var loc in aaContext.locations)
                        if (loc != null && loc.Keys[0] != null && loc.Keys[0] is string &&
                            !primaryKeyToCatalogEntry.ContainsKey((string)loc.Keys[0]))
                    ...
                    foreach (var assetGroup in groups)
                    {
                        if (aaContext.assetGroupToBundles.TryGetValue(assetGroup, out List<string> buildBundles))
                        {
                            ...
                            PostProcessBundles(assetGroup, buildBundles, outputBundles, results, aaContext.runtimeData,
                                aaContext.locations, builderInput.Registry, primaryKeyToCatalogEntry,
                                bundleRenameMap, postCatalogUpdateCallbacks);
                        }
                    }
                }

                ProcessCatalogEntriesForBuild(aaContext, Log, groups, builderInput, extractData.WriteData,
        ...
        Dictionary<AddressableAssetGroup, bool> m_SavedIncludeInBuildState =
            new Dictionary<AddressableAssetGroup, bool>();

        void CreateModOnlyBuildSetup(AddressableAssetSettings settings)
        {
            ...
            ExportConfig.Group.GetSchema<BundledAssetGroupSchema>().IncludeInBuild = true;
            
            // Set mod remote build path
            settings.profileSettings.SetValue(settings.activeProfileId, "Remote.LoadPath",
                ImportConstants.DynamicLoadPath);
        ...
        void CleanupModSetup(AddressableAssetSettings settings)
        {
            // revert status overwrite
            foreach (var group in settings.groups)
            {
                var schema = group.GetSchema<BundledAssetGroupSchema>();
            ...
            List<CachedAssetState> carryOverCachedState, Dictionary<string, string> bundleToInternalId)
        {
            ...
            using (var progressTracker = new UnityEditor.Build.Pipeline.Utilities.ProgressTracker())
            {
                ...
                Dictionary<string, ContentCatalogDataEntry> locationIdToCatalogEntryMap =
                    BuildLocationIdToCatalogEntryMap(aaContext.locations);
        ...
        {
            Dictionary<string, CachedAssetState> addressableEntryToCachedStateMap =
                new Dictionary<string, CachedAssetState>();
            ...
            AddressablesDataBuilderInput builderInput)
        {
            ...
            var retCode = ContentPipeline.BuildAssetBundles(buildParams, bundleBuildContent,
                out IBundleBuildResults result, buildTasks, Log);
        ...
        /// <returns></returns>
        protected virtual string ProcessGroupSchema(AddressableAssetGroupSchema schema,
            ...
            AddressableAssetsBuildContext aaContext)
        {
            ...
            List<string> uniqueNames =
                HandleDuplicateBundleNames(bundleInputDefs, aaContext.bundleToAssetGroup, assetGroup.Guid);
            m_OutputAssetBundleNames.AddRange(uniqueNames);
            m_AllBundleInputDefs.AddRange(bundleInputDefs);
            return string.Empty;
        }

        internal static List<string> HandleDuplicateBundleNames(List<AssetBundleBuild> bundleInputDefs,
        ...
        internal static string ErrorCheckBundleSettings(BundledAssetGroupSchema schema,
            AddressableAssetGroup assetGroup, AddressableAssetSettings settings)
        {
            ...
            {
                message = "BuildPath for group '" + assetGroup.Name +
                          "' is set to the dynamic-lookup version of StreamingAssets, but LoadPath is not. \n";
            }
            ...
            {
                Debug.LogWarningFormat("Bundle compression is set to LZMA, but group {0} uses local content.",
                    assetGroup.Name);
            }
        ...
        internal static string CalculateGroupHash(AddressableAssetGroup assetGroup)
        {
            // AF: always use group + project to caculate hash
            ...
            switch (packingMode)
            {
                case BundledAssetGroupSchema.BundlePackingMode.PackTogether:
                {
                    ...
                    GenerateBuildInputDefinitions(allEntries, bundleInputDefs,
                        CalculateGroupHash(assetGroup), "all", ignoreUnsupportedFilesInBuild);
                }
                    break;
                case BundledAssetGroupSchema.BundlePackingMode.PackSeparately:
                {
                    foreach (AddressableAssetEntry a in assetGroup.entries)
                    {
                        ...
                        combinedEntries.AddRange(allEntries);
                        GenerateBuildInputDefinitions(allEntries, bundleInputDefs,
                            CalculateGroupHash(assetGroup), a.address,
                ...
                case BundledAssetGroupSchema.BundlePackingMode.PackTogetherByLabel:
                {
                    ...
                    foreach (var entryGroup in labelTable)
                    {
                        ...
                        combinedEntries.AddRange(allEntries);
                        GenerateBuildInputDefinitions(allEntries, bundleInputDefs,
                            CalculateGroupHash(assetGroup), entryGroup.Key,
                            ignoreUnsupportedFilesInBuild);
                    }
                }
                    break;
        ...
        static string[] CreateRemoteCatalog(string jsonText, List<ResourceLocationData> locations,
        ...
        void PostProcessBundles(AddressableAssetGroup assetGroup, List<string> buildBundles, List<string> outputBundles,
            IBundleBuildResults buildResult, ResourceManagerRuntimeData runtimeData,
            List<ContentCatalogDataEntry> locations, FileRegistry registry,
            Dictionary<string, ContentCatalogDataEntry> primaryKeyToCatalogEntry,
            ...
            for (int i = 0; i < buildBundles.Count; ++i)
            {
                if (primaryKeyToCatalogEntry.TryGetValue(buildBundles[i], out ContentCatalogDataEntry dataEntry))
                {
                    ...
                    else
                    {
                        ...
                        string[] deconstructedBundleName = outputBundles[i]
                            .Substring(0, outputBundles[i].Length - extensionLength).Split('_');
                        string reconstructedBundleName =
                    ...
                    dataEntry.InternalId =
                        dataEntry.InternalId.Remove(dataEntry.InternalId.Length - buildBundles[i].Length) +
                        outputBundles[i];
        ...
        internal void AddPostCatalogUpdatesInternal(AddressableAssetGroup assetGroup, List<Action> postCatalogUpdates,
            ContentCatalogDataEntry dataEntry, string targetBundlePath, FileRegistry registry)
        {
            if (assetGroup.GetSchema<BundledAssetGroupSchema>()?.BundleNaming ==
                BundledAssetGroupSchema.BundleNamingStyle.NoHash)
            {
        ...
        protected virtual string ConstructAssetBundleName(AddressableAssetGroup assetGroup,
            BundledAssetGroupSchema schema, BundleDetails info, string assetBundleName)
        {
            ...
            {
                bundleNameWithHashing =
                    bundleNameWithHashing.Replace(".bundle", "_" + info.Hash.ToString() + ".bundle");
            }
